...
<%--
	Vue : Evan You (구글 => AngularJS:복잡하다)
		...
		사용 : MVVM
			  M (Model) : 데이터 저장 => data()
			  V (View) : 화면 출력
			  			{{}} , v-for , v-model , v-if , v-show, v-if else
			   	   ...
			   	   -------------------
			   	   	...
			   	   	data(){
			   	   		return {
			   	   			...
			   	   			isShow:true , Boolean
			   	   		}==> 선언 / 초기화 : 서버(Spring / NodeJS)읽기 불가능
			   	   	}
			   	   	-------------------
			   	   	 ViewModel => 데이터 처리
			   		1) 변수의 초기화
			   		  => 서버나 파일
			   		  => 이미 만들어진 메소드 (CallBack) => Vue 동작시 자동으로 호출
			   		  => componentDidMount()
			   		  mounted(){
